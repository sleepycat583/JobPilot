...
export function LowScoreReviewForm({ interrupt, disabled, onSubmit }: { interrupt: Extract<ThreadInterrupt, { type: 'low_match_score' }>; disabled: boolean; onSubmit: (command: LowScoreReviewCommand) => void }) {
  ...
  const [resumeVersion, setResumeId] = useState('')
  ...
  return <div>
    ...
    {!revising ? <div className="actions-row">
      ...
      <label htmlFor="resume-version">简历版本</label><input id="resume-version" value={resumeVersion} onChange={(event) => setResumeId(event.target.value)} disabled={disabled} />
      ...
      <button type="button" disabled={disabled || !canRevise} onClick={() => onSubmit({ action: 'revise_inputs', feedback: feedback.trim() || undefined, resume_id: resumeVersion.trim() || undefined, jd_text: jdText.trim() || undefined })}>提交修订</button>
    </div>}
  </div>
}