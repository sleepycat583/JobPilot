...
describe('InterviewAnswerForm', () => {
  it('submits answers and context with optional answer revision', async () => {
    ...
    await user.type(screen.getByLabelText('补充或更正背景'), '背景补充'); await user.click(screen.getByRole('button', { name: '提交补充' })); expect(onSubmit).toHaveBeenLastCalledWith({ action: 'context_update', context: '背景补充' }); expect(onSubmit.mock.lastCall?.[0]).not.toHaveProperty('answer')
