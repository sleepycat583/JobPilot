...
import { ArchiveRestore, DatabaseBackup, Download, HardDrive, LoaderCircle, LockKeyhole, RefreshCw, ShieldCheck, Trash2 } from 'lucide-react'
...
export function SettingsPage() {
  ...
  const cleanup = useMutation({
    ...
    onSuccess: () => {
      ...
      setConfirmation('')
      void queryClient.invalidateQueries({ queryKey: ['local-data-summary'] })
      void queryClient.invalidateQueries({ queryKey: ['local-cleanup-preview'] })
    },
  })
  ...
  return <div className="page-view settings-view">
    <div className="settings-heading"><div><span className="eyebrow">本地工作区</span><h2>隐私与数据</h2></div><button className="secondary-button" type="button" onClick={() => { void summary.refetch(); void preview.refetch() }}><RefreshCw size={16} />刷新状态</button></div>
    ...
    <section className="settings-band cleanup-band"><SectionTitle title="历史清理" meta="可预览" /><div className="cleanup-controls"><label htmlFor="retention-days">保留期限</label><select id="retention-days" value={retentionDays} onChange={(event) => setRetentionDays(Number(event.target.value))}>{retentionOptions.map((days) => <option key={days} value={days}>{days} 天</option>)}</select><button className="secondary-button danger" type="button" onClick={() => setCleanupOpen(true)} disabled={preview.isLoading}><Trash2 size={16} />清理历史</button></div><CleanupPreview preview={preview.data} loading={preview.isLoading} /></section>
...
function CleanupPreview({ preview, loading }: { preview: LocalCleanupPreview | undefined; loading: boolean }) {
  ...
  return <div className="cleanup-preview"><strong>{total} 条记录</strong><span>终态任务 {preview.terminal_jobs}</span><span>请求幂等记录 {preview.idempotency_records}</span><span>事件记录 {preview.execution_events}</span></div>
}
