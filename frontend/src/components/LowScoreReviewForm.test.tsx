...
describe('LowScoreReviewForm', () => {
  ...
  it('requires one valid revision input after entering the second step', async () => {
    ...
    expect(onSubmit).toHaveBeenCalledWith({ action: 'revise_inputs', feedback: undefined, resume_id: '2026-v2', jd_text: undefined })
  })
  it('accepts feedback-only revisions', async () => {
    ...
    expect(onSubmit).toHaveBeenCalledWith({ action: 'revise_inputs', feedback: '请重新检查项目经验', resume_id: undefined, jd_text: undefined })
  })
  it('requires at least twenty characters for JD-only revisions', async () => {
    ...
    expect(onSubmit).toHaveBeenCalledWith({ action: 'revise_inputs', feedback: undefined, resume_id: undefined, jd_text: '太短的岗位描述，要求具备稳定的后端开发经验。' })
  })
})